...
<head>
    ...
    <script type="text/javascript">
        function resultTrue(id,username,activityname) {
...
//                    confirmButtonColor:"#dd6b55",}, 红色
                function (inputValue) {
                    ...
                    console.log(xmlhttp);
                    // 2. 发送Ajax请求
                    xmlhttp.open("Post", "/apply-activity-admin/updateResultTrue?id=" + id + "&reason=" + inputValue + "&username=" + username + "&activityname=" + activityname, true)
                    xmlhttp.send();
                    // 3. 处理服务器响应
                    swal({
                        title: "提交成功",
...
//                    confirmButtonColor:"#dd6b55",}, 红色
                function (inputValue) {
                    ...
                    console.log(xmlhttp);
                    // 2. 发送Ajax请求
                    xmlhttp.open("Post", "/apply-activity-admin/updateResultFalse?id=" + id + "&reason=" + inputValue, true)
                    xmlhttp.send();
                    // 3. 处理服务器响应
...
<form action="/apply-activity-admin/obscure-match" method="post">
    <div class="formbody">
    ...
    <c:if test="${!empty list}">
        <c:forEach items="${list}" var="list" varStatus="vs">
            <tr>
                ...
                <td>${list.teamnumber}</td>
                <td>${list.activityname}</td>
                <td>${list.result}</td>
                <td>${list.reason}</td>
                <td>
                    <a href = "javascript:void(0);" onclick="resultTrue(${list.id},'${list.username}','${list.activityname}')"><font color="blue">通过</font></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<form action="/apply-activity-admin/apply-activity-list" method="get" rel="page">
